...
export const jobFormSchema = z.object({
    roles:z.string({required_error:'Job Title is required'}).min(80,{message:'Job Title must be at least 80 characters'}),
    jobType:z.enum(JOBTYPES, {required_error:'You need to select job type'}),
    salaryFrom: z.string({required_error:'Salary From is required'}),
    salaryTo: z.string({required_error:'Salary To is required'}),
    categoryId: z.string({required_error:'You need select category'}),
    ...
    responsibility: z.string({required_error:'Responsibility Description is required'}).min(10,{message:"Job description must be at least 10 characters"}),
    whoYouAre:  z.string({required_error:'Who You Are is required'}).min(10,{message:"Job description must be at least 10 characters"}) ,
    niceToHave:  z.string({required_error:'Nice to Have is required'}).min(10,{message:"Job description must be at least 10 characters"}) ,
    benefits:z.object({
        benefit:z.string(),
